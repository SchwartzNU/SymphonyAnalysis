...
for kk = 1:size(nodes,1)
  ...
  thisOutT = topOutputPos(topOutputPos(:,1)>=lx & topOutputPos(:,1)<=ux & topOutputPos(:,2)>=ly & topOutputPos(:,2)<=uy,:);
  thisOutB = botOutputPos(botOutputPos(:,1)>=lx & botOutputPos(:,1)<=ux & botOutputPos(:,2)>=ly & botOutputPos(:,2)<=uy,:);
  ...
  transformMat = lscov(quadData,thisIn); %changed from thisOut to thisIn 
