function nodes = localLSregistration_single(nodes,topInputPos,topOutputPos)
...
for kk = 1:size(nodes,1)
    ...
    %for totalOrd = 2:maxOrder
        for ord = 0:maxOrder % slightly more general - it can handle higher lateral polynomial orders
            quadData = [(thisIn(:,1).^ord).*(thisIn(:,2).^(maxOrder-ord)) quadData];
        end
    %end
    ...
    %for totalOrd = 2:maxOrder
        for ord = 0:maxOrder % slightly more general - it can handle higher lateral polynomial orders
            quadData = [(shiftedNode(1).^ord).*(shiftedNode(2).^(maxOrder-ord)) quadData];
        end
    %end
