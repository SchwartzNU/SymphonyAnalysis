...
sampleRate = cellData.epochs(epochIndices(1)).get('sampleRate');
...
%         response = response - mean(response);
        ...
        response = response + 0.03*(max(response) - min(response));
...
%         end
    ...
    if averageRepeatSeedEpochs
        ...
        stimulusFull = [stimulusFull, stimulus];
...
params_stim = NIM.create_stim_params([nLags 2 1], 'stim_dt', 1/frameRate);
...
nim = NIM(params_stim, [], [], 'spkNL', 'lin');
nim = nim.add_subunits('lin', 1);
nim = nim.set_reg_params('d2t', 10);
nim = nim.set_reg_params('d2x', 10);
...
numFittingLoops = 2;
nim = nim.set_reg_params('d2t', 40);
nim = nim.set_reg_params('d2x', 40);
...
for si = 1:numSubunits
    f = reshape(nim.subunits(si).filtK, [], 2);
    for fi = 1:size(f,2)
        c = colorsBySubunit(si,:) - .2 * (fi-1); % plot spatial filters a little darker
        c(c < 0) = 0;
        h(si,fi) = plot(filterTime, f(:,fi), 'Color', c);
        hold on
    end
end
