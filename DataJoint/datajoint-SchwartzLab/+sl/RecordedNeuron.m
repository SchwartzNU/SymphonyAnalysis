    ...
    methods(Access=protected)
        function makeTuples(self,key)
            ...
            for n=1:N_cellData
                ...
                s.channel = channel;
                s.cell_data = curName;
                %disp(['curName: ' curName])
                for i=1:length(k)
                    dataset_name = strrep(k{i}, '.', '_dot_'); %make sure dataset_name is legal
                    s.dataset_name = dataset_name;
                    s.epoch_ids = datasetsMap(k{i});
                    %now populate the epochs in each dataset
                    allModes = cell(1,length(s.epoch_ids));                   
                    for e=1:length(s.epoch_ids)
                        epoch_insert_error = false;
                        epoch_init_struct = struct;
                        epoch_init_struct.cell_id = key.cell_id;
                        epoch_init_struct.number = s.epoch_ids(e);
                        epoch_init_struct.cell_data = s.cell_data;
                        epoch_init_struct.protocol_params = struct;
                        epoch_init_struct.raw_data_filename = raw_data_filename;
                        %load required epoch values
                        ep = cellData.epochs(s.epoch_ids(e));
                        ...
                        try
                            ...
                            disp(['DataLink not found for ' epoch_init_struct.cell_data ' epoch: ' num2str(s.epoch_ids(e))]);
                    ...
                    if length(unique_modes) == 1
                       s.recording_type = unique_modes{1};                 
                    else 
                       s.recording_type = 'Multi';
                    end
                    insert(sl.Dataset, s, 'REPLACE');
                end
