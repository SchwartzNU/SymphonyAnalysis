function results = SpikeDetector_simple_Iclamp(D, SampleInterval, thres_std)
HighPassCut_spikes = 200; %Hz, in order to remove everything but spikes
ref_period = 2E-3; %s
...
for i=1:Ntraces
    ...
    if ~isempty(peaks) && max(D(i,:)) > min(D(i,:)) %make sure we don't have bad/empty trace
        ...
        if isempty(spike_ind)
            ...
            spikeAmps{i} = spike_peaks./noiseLevel;
